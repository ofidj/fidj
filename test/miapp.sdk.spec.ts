...
describe('miapp.sdk', () => {
    ...
    const _login = 'miappTest' + _generateObjectUniqueId();
    const _password = 'miappPassword';
    ...
    describe('internal.service', () => {
        ...
        it('should miappInit KO : without required miappId', function (done) {
            ...
            srv
                ...
                .catch(function (err) {
                    expect(err.equals(new FidjError(400, 'Need a miappId'))).toBeTruthy();
                    done();
                });

        });

        it('should miappInit KO : reject no connection', function (done) {

            const srv = new InternalService(_log, _q);
            spyOn((srv as any).connection, 'verifyConnectionStates').and.returnValue(_q.reject('no connection'));

            srv
                .miappInit('testAppWithVerifyReject')
                .then(() => {
                    done.fail('should fail');
                })
                .catch(function (err) {
        ...
        it('should miappInit KO : no endpoint at all', function (done) {
            ...
            srv
                .miappInit('testAppWithoutEndpoint')
                .then(() => {
                    done.fail('should fail');
                })
                .catch(function (err) {
        ...
        it('should miappInit OK : got old endpoint and is login', function (done) {

            const srv = new InternalService(_log, _q);

            spyOn((srv as any), 'miappIsLogin').and.returnValue(true);
            spyOn((srv as any).connection, 'verifyConnectionStates').and.returnValue(_q.resolve());
        ...
        it('should miappInit OK : with endpoints several times (prod or not)', function (done) {
            ...
            srv
                .miappInit('testAppProd')
                .then(function (value) {
                    expect(value).toBeUndefined();
                    expect((srv as any).connection.verifyConnectionStates).toHaveBeenCalledTimes(1);
                    expect((srv as any).connection.getApiEndpoints).toHaveBeenCalledTimes(2);
                    expect((srv as any).connection.setClient).toHaveBeenCalledTimes(1);
                    return srv.miappInit('testAppNotProd', {prod: false});
                })
                .then(function (value) {
                    expect(value).toBeUndefined();
        ...
        it('should miappLogin KO : without initialisation', function (done) {

            const srv = new InternalService(_log, _q);
            srv
                .miappLogin(_login, _password)
                .then(function (user) {
        ...
        it('should miappLogin OK', function (done) {
        ...
        it('should miappLoginInDemoMode OK', function (done) {
        ...
        it('should miappLogout OK : without initialisation', function (done) {
        ...
        it('should miappLogout OK', function (done) {

            const srv = new InternalService(_log, _q);
            (srv as any).connection.miappId = 'mockedId';
        ...
        it('should miappSync OK : without connection', function (done) {
            const srv = new InternalService(_log, _q);
            (srv as any).connection.miappId = 'mockedId';
        ...
        it('should miappSync KO : with a fail emptyFn', function (done) {
            const srv = new InternalService(_log, _q);
            (srv as any).connection.miappId = 'mockedId';
            spyOn((srv as any).connection, 'getDBs').and.returnValue([]);
            spyOn((srv as any).session, 'setRemote').and.returnValue(null);
            ...
            const mock = {
                launchIfEmpty: (args: any) => {
                    return srv.miappPutInDb('test');
                }
            };
            spyOn(mock, 'launchIfEmpty').and.callThrough();

            srv
                .miappSync(mock.launchIfEmpty, 'noarg')
                .then(() => {
                    done.fail();
                })
                .catch(function (err) {
        ...
        it('should miappSync OK', function (done) {
            const srv = new InternalService(_log, _q);
            (srv as any).connection.miappId = 'mockedId';
            spyOn((srv as any).connection, 'getDBs').and.returnValue([]);
            spyOn((srv as any).session, 'setRemote').and.returnValue(null);
        ...
        xit('should miappRemoveInDb TODO', function (done) {

            done.fail('todo');
        });

        it('should miappPutInDb & miappFindInDb & miappFindAllInDb OK', function (done) {
            const srv = new InternalService(_log, _q);
            (srv as any).connection.miappId = 'mockedId';
            ...
            srv
                .miappPutInDb('testDataToEncrypt')
                .then(function (value) {
        ...
        it('should miappPutInDb with miappCrypto OK', function (done) {
            const srv = new InternalService(_log, _q);
            spyOn((srv as any).connection, 'getClientId').and.returnValue('id');
            spyOn((srv as any).session, 'isReady').and.returnValue(true);
            spyOn((srv as any).session, 'put').and.returnValue(Promise.resolve('ok'));
            spyOn((srv as any), '_generateObjectUniqueId').and.returnValue('uid');
            (srv as any).connection.miappCrypto = true;
        ...
        it('should miappGetEndpoints', function () {
            ...
            expect(srv.miappGetEndpoints().length).toBe(0);
            ...
            getAccessPayload.and.returnValue(JSON.stringify(accessPayload));
            expect(srv.miappGetEndpoints().length).toBe(2);
            expect(JSON.stringify(srv.miappGetEndpoints())).toBe(JSON.stringify(accessPayload.endpoints));
            ...
            getAccessPayload.and.returnValue(JSON.stringify(accessPayload));
            expect(srv.miappGetEndpoints().length).toBe(2);
            expect(srv.miappGetEndpoints()[1].key).toBe('my endpoint3');
            expect(!srv.miappGetEndpoints()[1].blocked).toBe(true);

            // 3 endpoints , one blocked & filter on key (showBlocked = false by default)
            filter = {key: 'my endpoint3'};
            getAccessPayload.and.returnValue(JSON.stringify(accessPayload));
            expect(srv.miappGetEndpoints(filter).length).toBe(1);
            expect(srv.miappGetEndpoints(filter)[0].key).toBe('my endpoint3');
            expect(!srv.miappGetEndpoints(filter)[0].blocked).toBe(true);

            // 3 endpoints , one blocked & filter on key blocked (showBlocked = false by default)
            filter = {key: 'my endpoint2'};
            getAccessPayload.and.returnValue(JSON.stringify(accessPayload));
            expect(srv.miappGetEndpoints(filter).length).toBe(0);

            // 3 endpoints , one blocked & filter bypass showBlocked
            filter = {showBlocked: true};
            getAccessPayload.and.returnValue(JSON.stringify(accessPayload));
            expect(srv.miappGetEndpoints(filter).length).toBe(3);
            expect(srv.miappGetEndpoints(filter)[1].key).toBe('my endpoint2');
            expect(!srv.miappGetEndpoints(filter)[1].blocked).toBe(false);

            // 3 endpoints , one blocked & filter on key and bypass showBlocked
            filter = {key: 'my endpoint2', showBlocked: true};
            getAccessPayload.and.returnValue(JSON.stringify(accessPayload));
            expect(srv.miappGetEndpoints(filter).length).toBe(1);
            expect(srv.miappGetEndpoints(filter)[0].key).toBe('my endpoint2');
            expect(!srv.miappGetEndpoints(filter)[0].blocked).toBe(false);

        });

        it('should miappPostOnEndpoint KO : 400 without valid endpoint', function (done) {
            ...
            accessPayload = {endpoints: []};
            getAccessPayload.and.returnValue(JSON.stringify(accessPayload));
            srv.miappPostOnEndpoint('none', {})
                .then(result => done.fail('should fail'))
                .catch((err: ErrorInterface) => {
                    expect(err.code).toBe(400);
                    expect(err.reason).toBe('miapp.sdk.service.miappPostOnEndpoint : endpoint does not exist.');
                    done();
                })
        });

        it('should miappPostOnEndpoint OK', function (done) {
            ...
            getAccessPayload.and.returnValue(JSON.stringify(accessPayload));
            srv.miappPostOnEndpoint('my endpoint1', {mock: true})
                .then(result => {
