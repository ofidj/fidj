...
describe('miapp.tools.LocalStorage', () => {
...
it('should set an XML item',  () => {
    ...
    var xml = miapp.Xml.string2Xml('<span><div><p/></div></span>');
    var v = fs.set('a', xml);
    ...
    expect(miapp.Xml.xml2String(v)).toEqual('<span><div><p/></div></span>');
...
var fs = null;
beforeEach( () => {
    LocalStorage = new miapp.LocalStorageFactory(window.localStorage);
    fs = new LocalStorage();
});
...
var fs = null;
beforeEach( () => {
    LocalStorage = new miapp.LocalStorageFactory(new miapp.MemoryStorage());
    fs = new LocalStorage();
});
