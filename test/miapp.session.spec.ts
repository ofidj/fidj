    ...
    describe('Session', () => {
        ...
        it('should put string', (done) => {
            ...
            (session as any).db.put = (data, callback) => {
                expect(data.miappData).toBe('{"string":"mockdata"}');
        ...
        it('should put json without keyword (_id,json,...)', (done) => {
            ...
            (session as any).db.put = (data, callback) => {
                ...
                expect(data.miappData).toBe('{"json":{"test":"test val","data":"data val"}}');
                const response = {ok: true, id: 'mockid', rev: 'mockrev'};
                callback(null, response);
            };
        ...
        it('should put json', (done) => {
            ...
            (session as any).db.put = (data, callback) => {
                expect(data._id).toBe('_id');
                expect(data.miappData).toBe('{"json":{"test":"mockjson"}}');
                const response = {ok: true, id: 'mockid', rev: 'mockrev'};
                callback(null, response);
            };
        ...
        it('should put multiple json and update _rev', (done) => {
            ...
            (session as any).db.put = (data, callback) => {
                ...
                if (data._id === '_id1') {
                    ...
                    expect(data.miappData).toBe('{"json":{"test":"mockjson1"}}');
                    response = {ok: true, id: 'mockid1', rev: 'mockrev1'};
                } else {
                    expect(data._id).toBe('_id2');
                    expect(data.miappData).toBe('{"json":{"test":"mockjson2"}}');
        ...
        it('should put json encrypted', (done) => {
            ...
            (session as any).db.put = (data, callback) => {
                expect(data._id).toBe('_id');
                expect(data.miappDacr).toBe('encrypted');
                const response = {ok: true, id: 'mockid', rev: 'mockrev'};
                callback(null, response);
            };
            const obj = {
        ...
        it('should get string', (done) => {
            ...
            (session as any).db.get = (id) => {
                expect(id).toBe('_id');
                const row = {_id: '_id', _rev: '_rev', miappData: '{"string":"mockdata"}'};
        ...
        it('should get json', (done) => {
            ...
            (session as any).db.get = (id) => {
                expect(id).toBe('_id');
                const row = {_id: '_id', _rev: '_rev', miappData: '{"json":{"id":"mockjson"}}'};
                return Promise.resolve(row);
            };
            session.get('_id')
                .then((result) => {
        ...
        it('should get json encrypted', (done) => {
            ...
            (session as any).db.get = (id) => {
                expect(id).toBe('_id');
                const row = {_id: '_id', _rev: '_rev', miappDacr: 'to_decrypt'};
                return Promise.resolve(row);
            };
            const obj = {
                decrypt: () => {
        ...
        it('should getAll mixed and encrypted', (done) => {
            ...
            (session as any).db.allDocs = () => {
                const rows = [];
                const row1 = {_id: '_id1', _rev: '_rev1', miappDacr: '{"string":"to_decrypt1"}'};
                const row2 = {_id: '_id2', _rev: '_rev2', miappDacr: '{"json":{"id":"to_decrypt2"}}'};
                const row3 = {_id: '_id3', _rev: '_rev3', miappDacr: '{"number":"to_decrypt3"}'};
                rows.push({doc: row1});
                rows.push({doc: row2});
                rows.push({doc: row3});
